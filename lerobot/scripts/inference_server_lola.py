...
class InferenceServer:
    ...
    def receive_observation(self, conn: socket.socket) -> dict:
        ...
        for key in ["cam_high", "cam_left_wrist", "cam_right_wrist"]:
            ...
            """
            # This is all unnecessary, since Lola simply resizes to 224x224 directly, instead of maintaining aspect ratio first
            # and then center-cropping.

            # Resize to that the resized height is self.image_size, and the width is scaled accordingly.
            wpercent = (self.image_size / float(img.size[1]))
            wsize = int((float(img.size[0]) * float(wpercent)))
            new_size = (wsize, self.image_size)
            resized_img = img.resize(new_size, Image.Resampling.LANCZOS)

            # Center-crop to self.image_size x self.image_size
            left = (new_size[0] - self.image_size ) // 2
            top = (new_size[1] - self.image_size ) // 2
            right = (new_size[0] + self.image_size ) // 2
            bottom = (new_size[1] + self.image_size ) // 2
            new_img = resized_img.crop((left, top, right, bottom))
            """

            new_img = img.resize((self.image_size, self.image_size), Image.Resampling.LANCZOS)

            if cam_remapping[key] == "primary":
                # Reset the observation history if the task has changed
                if obs["task"] != self.prev_task:
                    self.primary_cam_history = []
                    self.prev_task = obs["task"]

                self.primary_cam_history.append(new_img)
                if len(self.primary_cam_history) > OBS_SEQ_LEN:  # Keep last 3 frames
                    self.primary_cam_history.pop(0)
                obs[cam_remapping[key]] = list(self.primary_cam_history)
            else:
                obs[cam_remapping[key]] = new_img
