...
def zero_action_filter(traj: dict) -> bool:
    ...
    ACTION = traj["action"][:, :6]  # shape: [batch_size, 6]
    
    DROID_NORM_0_ACT = 2 * (torch.zeros_like(ACTION) - DROID_Q01) / (DROID_Q99 - DROID_Q01 + 1e-8) - 1
    
    # 比较动作与归一化 0 的差异（返回布尔值）
    diff = torch.abs(ACTION - DROID_NORM_0_ACT) > 1e-5

    return torch.any(diff).item()
...
OXE_DATASET_CONFIGS = {
    ...
    # Calvin
    "calvin_abc_d": {
        "image_obs_keys": {"primary": "primary", "secondary": "secondary", "wrist": None},
        "depth_obs_keys": {"primary": None, "secondary": None, "wrist": None},
        "state_obs_keys": ["EEF_state", None, "gripper_state"],
        "state_encoding": StateEncoding.POS_EULER,
        "action_encoding": ActionEncoding.EEF_POS,
