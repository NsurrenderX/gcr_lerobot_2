...
class PaliGemmaWithExpertModel(PreTrainedModel):
    ...
    def __init__(self, config: PaliGemmaWithExpertConfig, init_load = False, init_path = None):
        ...
        self.kv_repre = nn.ModuleList([KvRepresentation(hidden=128, in_head=[6, 2], out_head=2) for _ in range(self.num_layers)])
        
        self.kv_mask = nn.ModuleList([KvLearnableMask(hidden=128, in_head=[6, 2]) for _ in range(self.num_layers)])
